...
| 请求超时 | 每次请求等多久（默认 30） |
| 重试次数 | 单个请求失败后重试几次（默认 3）。每次尝试最多等「请求超时」秒（如直连被 RST，每次都要等满 30s 超时）；重试之间按 2s / 5s / 8s… 递增退避（第 n 次等待约 `n*3+2` 秒）。超时 30s × 重试 3 次时，单个请求最坏耗时约 97s。设置页重试拉动条上限为 3（2 / 3 二档可选），超时 30s + 重试 3 次，为保检测速度 |
