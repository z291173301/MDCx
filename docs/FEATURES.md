...
| r18dev | r18.dev | JSON API，番号自动补零（仅能有码） |
...
> 注意：javdb_api、javdb_app、missav_api、r18dev、thejavdb_api 这五条是免 CF 通道，稳定性好，建议优先选用。
...
写入时可通过 **本地NFO合并策略**（软件界面读取模式区域：勾选框 + 下拉框，5 选 1）控制如何处理已存在的 NFO：勾选后按所选策略合并——全新数据优先 / 本地信息优先 / 合并数组去重 / 保留已有字段 / 仅填补空字段；不勾选则直接用新数据覆盖、不合并本地 NFO，防止重刮覆盖手动整理的内容（如手改的简介、标签）。
