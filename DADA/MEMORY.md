...
- Instructions:
  ...
  - **本机（Windows）源码验证环境（2026-09-22 实证）**：系统 Python 在 `C:\Program Files\Python314`，ruff / pytest / PyQt6 / pydantic 齐全；**未装 uv / mypy**（`quick-check` 的 mypy 段跑不了、`check.py` 的 `uv run` 类路径受限，属环境缺失非代码问题）。三条沙箱陷阱：①OpenCode 的 shell 直接调该 python 会触发 stdout 捕获异常（`StandardOutputEncoding is only supported when standard output is redirected` 或无输出）——一律写 `.bat`、`cd /d <repo>`、命令内 `> file 2>&1` 落盘后读文件；②**`.bat` 里 `set VAR=value` 不会传到子 python**（连 `PYTHONUTF8`/`PYTHONIOENCODING` 都进不去）——要注入环境变量必须写 Python 包装脚本 `os.environ[...]=...` 后 `pytest.main(...)`；③中文 Windows 子进程默认 GBK，`test_build_script` 的 `print('中文✅')` 会 UnicodeEncodeError——本地全绿需 `PYTHONIOENCODING=utf-8`（CI Windows job 就是这么设的），但受②只能走包装脚本；④**不要为跑 `test_ui_structure` 去 `uv run`**：uv 会 sync 出 ~650MB `.venv` 且其 `python.exe` 在沙箱里起不来（`os error 740`/无法创建进程），只有 `uv.exe` 能跑 ruff。pytest 成功时 conftest `pytest_sessionfinish` 会 `os._exit(0)` 丢弃 stdout，**判定只认退出码**（多失败时因收尾段错误会退非 0/负值），要细节就加 `--junitxml`。
